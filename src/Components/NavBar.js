...
function NavigationBar() {
 ...
 const {upcomingRides , setsortedRides ,nearestRides,lastRides,rides} = RideContextProvoider()
  ...
  return (
    ...
    <div className="actionbuttons">
        <div className="Rides">
          ...
          <p onClick={()=>setsortedRides(upcomingRides)}>Upcoming rides {upcomingRides ?<>({upcomingRides.length})</>:""}</p>
          <p onClick={()=>setsortedRides(lastRides)}>Last rides {lastRides ?<>({lastRides.length})</>:""}</p>
        </div>
        <div className="filter">
          <img className="filter-logo" src={FILTERLOGO} />
          <p onClick={()=>setdropDown(!dropDown)}>Filter</p>
          <div style={dropDown ? {display:'block'}:{display:'none'}} className='dropdown'>
            <span>Filters</span>
            <hr/>
